...
def m(c,x):
 ...
 if c==',':return np.reshape(x,1)if ax(x)else np.matrix(np.reshape(x,(1,)+x.shape))if len(x.shape)==1 \
                  else None
 if c=='@':
  try:return x[0]
...
def d(c,a,x):
 ...
 if c==',':return np.concatenate((m(',',a)if ax(a)else a,m(',',x)if ax(x)else x))
