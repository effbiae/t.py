...
t_=lambda x:(print(x,tx(x)),i0)[-1]
i=6;f=13;ty=lambda x:ct.c_int if tx(x)==i else e2 if tx(x)==f else i0 if tx(x)==1 else t_(x);pa=lambda x:ix(x)if tx(x)==i else ex(x) if tx(x)==f else gx(x)
