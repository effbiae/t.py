...
k=lambda i,x,y:d(a.k,[U,i2,U,U])(i,0 if x is None else x,y)
...
import numpy as np
pk=lambda x:None if x==0 else pa(x)if ax(x) else [np.copy(np.ctypeslib.as_array(ct.cast(sx(x),ct.POINTER(ty(x))),shape=sh(x))),_r(x)][0]
a.m_(0,0);k_(0,0)
