...
a_ke=d(a.ke,[U,i2,U,U]);k=lambda i,x,y:a_ke(i,0 if x is None else x,y)
ax=d(a.kax,[i2,U]);tx=d(a.ktx,[i2,U]);mx=d(a.kmx,[i2,U]);nx=d(a.knx,[i2,U]);gx=d(a.kgx,[i0,U]);px=d(a.kpx,[i2,U]);ux=d(a.kux,[i2,U])
_r=lambda x:k_(0,x);r_=lambda x:k_(1,x);sx=lambda x:k_(2,x);"rr=lambda x:k_(3,x)if not ax(x)else 'a'"
