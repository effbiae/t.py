import ctypes as ct;import numpy as np;U=ct.c_ulonglong;i2=ct.c_uint;e2=ct.c_float;i0=ct.c_uint8;P=open("P").read();a=ct.CDLL('./liba.so')
def d(f,s):f.restype=s[0];f.argtypes=s[1:];return f
k_=d(a.k_,[U,U,U]);m_=d(a.m_,[U,i2,U]);ti=d(a.kti,[U,i2]);te=d(a.kte,[U,e2]);iz=d(a.kiz,[ct.c_int,U]);ez=d(a.kez,[e2,U])
...
ndt=lambda t:np.ctypeslib.ndpointer(ty(t),flags="C_CONTIGUOUS");kcze=d(a.kcze,[U,ndt(f),U]);kczi=d(a.kczi,[U,ndt(i),U])
sh=lambda z:0 if az(z)else(uz(z),round(nz(z)/uz(z)if uz(z)else nz(z)))if mz(z)else(nz(z),)
err=lambda z:pz(z);a=lambda z:np.array(iz(z)if tz(z)==i else ez(z) if tz(z)==f else gz(z))
g=lambda z,t,s:[(kcze if t==f else kczi)(a:=np.ones(s,dtype=np.float32 if t==f else np.int32),z),_r(z),a][-1]
